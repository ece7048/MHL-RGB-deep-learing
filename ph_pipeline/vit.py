...
#Extention Author: Michail Mamalakis
#Version: 0.1
#Licence:
...
class Dense(tf.keras.Model):
    ...
    def call(self, x,x_past):
        return self.fn(x) + x +x_past
...
class Transformer(tf.keras.Model):
    ...
    def __init__(self, dim, depth, heads, mlp_dim, model_case):
        super().__init__()
        layers = []
        if model_case=='res':
        ...
        if model_case=='dense':
            for _ in range(depth):
                layers.extend([
                    Residual(PreNorm(dim, Attention(dim, heads = heads))),
                    Dense(PreNorm(dim, FeedForward(dim, mlp_dim)),PreNorm(dim, Attention(dim, heads = heads)))
                ])


        self.net = tf.keras.Sequential(layers)
#TODO residual linking and Dense linking of layers
...
class ViT(tf.keras.Model):
    ...
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels=3, model_case='res'):
        super().__init__()
        ...
        self.transformer = Transformer(dim, depth, heads, mlp_dim, model_case)
    ...
    def call(self, img):
        shapes = tf.shape(img)
        print(img.shape)
        print("image ViT shape input: ", shapes)
