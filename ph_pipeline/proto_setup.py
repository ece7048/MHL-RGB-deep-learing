...
from ph_pipeline import proto_few_shot, proto_data, fewshots_proto
from ph_pipeline.proto_few_shot import Prototypical
from ph_pipeline.proto_data import load
from ph_pipeline.fewshots_proto import TrainEngine
...
def train(config):
    ...
    #model_dir= './Model/omniglot1234_trainval.h5'
  
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    data_dir1 =config['data.dataset']
    data_dir="./../../data/"+data_dir1
    w,h,c =list(map(int, config['model.x_dim'].split(',')))
    dim=[w,h,c]
    ret = load(data_dir, config, ['train', 'val'],dimen=dim)
