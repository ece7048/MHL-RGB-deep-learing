...
def load(data_dir, config, splits, dimen=[28,28,1]):
    """
    ...
    elif config['data.dataset'] == "fewshot":
        ds = load_covid(data_dir, config, splits, dimen=dimen)
    else:
...
class DataLoader2(object):
    def __init__(self, data, n_classes, n_way, n_support, n_query,dimen=[28,28,1]):
        self.data = data
        self.n_way = n_way
    ...
    def get_next_episode(self):
        n_examples = 20
        support = np.zeros([self.n_way, self.n_support, self.h, slef.w, self.c], dtype=np.float32)
        query = np.zeros([self.n_way, self.n_query, self.h, slef.w, self.c], dtype=np.float32)
...
def load_and_preprocess_image(img_path, rot, dim=[28,28]):
    """
    ...
    """
    #print(img_path)
    img = Image.open(img_path).resize((dim[0], dim[1])).rotate(rot)
    img = np.asarray(img)
    img = 1 - img
    return np.expand_dims(img, -1)


def load_class_images(n_support, n_query, img_paths, rot, dimm=[28,28]):
    """
    ...
    for i in range(n_support):
        img = load_and_preprocess_image(support_paths[i],dim=[dimm[0].dimm[1]])
    ...
    for i in range(n_query):
        img = load_and_preprocess_image(query_paths[i],dim=[dimm[0].dimm[1]])
...
def load_omniglot(data_dir, config, splits):
    ...
    for split in splits:
        ...
        print('class number, nway, ... data shape')
        print(len(classes),n_way)
        print(np.array(data).shape)
        data_loader = DataLoader2(data,
                                 n_classes=len(classes),
                                 n_way=n_way,
                                 n_support=n_support,
                                 n_query=n_query)
        print('the data number are: support ... query')
        print(n_support,n_query)
        ret[split] = data_loader
    return ret

def load_covid(data_dir, config, splits, dimen=[28,28,1]):
    """
    Load omniglot dataset.
    Args:
        data_dir (str): path of the directory with 'splits', 'data' subdirs.
    ...
    for split in splits:
        ...
        data = np.zeros([len(classes), len(img_paths[0]), dimen[0], dimen[1], dimen[2]])
        print(dimen,len(classes))
        for i_class in range(len(classes)):
            print('classe:',i_class)
            print('path: ', img_paths[i_class])
            print('rotation: ',img_paths[i_class])
            for i_img in range(len(img_paths[i_class])):
                data[i_class, i_img, :, :,:] = load_and_preprocess_image(img_paths[i_class][i_img], rotates[i_class],dim=[dimen[0],dimen[1]])
        print('class number, nway, ... data shape')
        print(len(classes),n_way)
        print(np.array(data).shape)
        data_loader = DataLoader2(data,
